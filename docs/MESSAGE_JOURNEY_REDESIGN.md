...
## Decision record — the direct-only signal rule (settled 2026-09-02, shipped in 9dbdfb6f)

**Rule. A signal figure is the sender's only where an observer heard the sender's own radio with no repeater in between. Everywhere else the app shows no figure at all.**

Every SNR and RSSI the observer network reports is measured at the observer, on the last leg into it. When the observation's hop list is non-empty, that leg ran from a repeater's transmitter to the observer's receiver. The sender is upstream of it and contributes nothing to it. A row reading "Spyglass Observer 2.0 · 13.5 dB · 1 hop" was therefore grading a link between two third parties and presenting it as a measure of how well the message got out — and the four-bar glyph beside it, the green through red colour, the "best 13.5 dB" headline and the "Strongest signal" sort all inherited the same error. Rafael identified this from a live screen before the spec did.

What the rule governs, permanently:

- `PacketScopeReception.directSNR` is the SNR of the route whose hop list is empty, and is the only per-observer figure any surface may print. `bestSNR` remains as the raw fold for tests and is not fit to show.
- `PacketScopeReception.directRSSI` is `bestRSSI`, and only when *every* one of that observer's receptions was direct: RSSI is folded across observations with no route attribution, so a mixed observer cannot claim it.
- `PacketScopeSummary.bestDirectSNR` is the only summary figure shown. When no observer heard the sender directly, the network measured nothing about the sender's transmitter and the screen says nothing about it.
- Signal bars, signal colour and decibels are absent from any row whose chain has repeaters in it. The slot is kept so names stay aligned; it is not filled with a neutral glyph, which would read as a weak signal rather than as no measurement.
- Route ranking (`PacketScopeCoverageBuilder.outranks`) leads on hop count. SNR only breaks ties between routes of equal length. Ranking a 3-hop route above a direct one for a louder number ranked a repeater's link above the sender's own.
- There is no "strongest signal" sort.
- `packetScope.signalFooter` explains the omission, and only when some row is actually missing a figure.

Still open under this rule, and folded into question 3 below: the map's observer legs are still stroked by `MapLine.LineStyle.forSNR` and still carry `snrBadge` readouts built from the same measurement. The rule as shipped governs the panel; the map has not been brought under it.
