...
public struct AggregatedCell: Sendable {
  ...
  public var txSnrSum: Double = 0
  public var txSnrCount: Int = 0
  /// Best uplink reading in the cell — the "can I get out of here" number: coverage
  /// is a property of the best usable link, not the average of everything overheard.
  ...
  public var isDeadZone: Bool {
    packetCount == 0 && probesSent > 0
  }
}
...
public enum CellAggregator {
  ...
  public static func fold(_ sample: SurveySample, into cell: inout AggregatedCell) {
    ...
    if let txSnr = sample.txSnr {
      cell.txSnrSum += txSnr
      cell.txSnrCount += 1
      cell.maxTxSnr = Swift.max(cell.maxTxSnr ?? txSnr, txSnr)
    }
