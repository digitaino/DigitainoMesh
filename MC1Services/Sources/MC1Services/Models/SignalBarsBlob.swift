...
/// Wire-format snapshot of the firmware's live repeater signal table, fetched from the
/// signal-bars sync slot on Digitaino custom firmware.
///
/// The device builds this on demand from its in-memory `_signals[]` table (it is
/// *not* persisted to flash), so every fetch returns a fresh view.
///
/// The signal-bars service uses this in *viewer mode* (custom firmware present): the
...
/// - v1 entry (8 bytes): `[id][rx_x4][tx_x4][flags][age_s(LE16)][rtt_ms(LE16)]`
/// - v2 entry (11 bytes): `[id_len][h0][h1][h2][rx_x4][tx_x4][flags][age_s(LE16)][rtt_ms(LE16)]`
///
...
public struct SignalBarsBlob: Sendable, Equatable {
  ...
  public struct Entry: Sendable, Equatable {
    ...
    public var rxSnr: Double? {
      hasRx ? Double(rxSnrX4) / 4.0 : nil
    }

    /// TX SNR in dB, or `nil` if no TX measurement.
    public var txSnr: Double? {
      hasTx ? Double(txSnrX4) / 4.0 : nil
    }

    /// Hex repeater id at the advertised hash size (e.g. "0C", "0C13", "0C13AB").
    public var hexID: String {
      Data(idHash.isEmpty ? [id] : idHash).uppercaseHexString()
    }
  }

  /// Schema version for forward compatibility. Decoding accepts `1` and `2`; encoding emits `2`.
  ...
  /// Parses a blob previously fetched from the signal-bars sync slot.
  ...
  /// Serializes to the firmware wire format (always version 2). The device is normally the
  /// producer; the app only decodes — this exists mainly for round-trip tests.
  ...
  ///
  /// Used by the signal-bars engine (stock firmware) to pick the best repeater the same
  /// way the device does; viewer mode trusts the device's `is_best` flag directly.
