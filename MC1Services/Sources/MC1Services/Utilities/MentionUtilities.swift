...
public enum MentionUtilities {
  ...
  public static let mentionRegex: NSRegularExpression? = try? NSRegularExpression(pattern: mentionPattern)
  ...
  private static let leadingMentionRegex: NSRegularExpression? = try? NSRegularExpression(pattern: #"^@\[[^\]]+\]\s*"#)
  ...
  public static func buildReplyText(mentionName: String, messageText: String, draft: String = "") -> String {
    let previewSource: String = if let regex = leadingMentionRegex,
      ...
      String(messageText[matchRange.upperBound...])
    } else {
      messageText
    }
