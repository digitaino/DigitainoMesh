...
- **Adaptive TX Power** — Automatic transmit power management that starts at a user-configured base level and escalates when messages aren't repeated, then holds steady at the working level. Supports external power amplifier offsets (e.g., WisMesh Pocket 1W +8 dB) with EIRP-aware step selection. Radio chip power is verified with retry logic, and the toolbar shows a live TX power capsule color-coded by escalation state (green = base, orange = elevated, red = max). Users can manually override power from the signal bars popover, and power resets on new conversations or disconnect
- **Community Signal Map** — Crowd-sourced mesh coverage map. Survey data from all contributors is aggregated into a hex-grid overlay with SNR-based color coding. The iOS app shows a dedicated Community Map view and a toggleable hex overlay on the main map, both auto-refreshing every 15 seconds. Server component lives in a [separate repository](https://github.com/digitaino/meshmap)
...
- **Shareable Route Links** — When you "Reply with Route", the route data is uploaded and a short URL is generated. The link opens an interactive web page showing the route plotted through repeaters with hop-by-hop annotations, a polyline connecting located hops, and a collapsible details panel
...
- **Path Map Generator** — Enter hex IDs manually (from message paths) to visualize repeater routes on an interactive map. Supports 2-6 character hex hashes and full 64-character public keys. Share the path as a web link. Chat messages containing hex path chains are auto-detected with an inline card
...
- **Signal Survey (Wardriving)** — Record mesh signal quality as you move, building a coverage heatmap. Start a GPS-tagged survey session that pairs every received packet with your location. Visualize data as individual points or an aggregated hex-grid heatmap with SNR-based color coding. Tap any hex cell to see signal stats in a native bottom sheet. Every probe automatically sends discover requests (zero-hop, no flood) and trace packets (hop-by-hop, naturally limited) for TX SNR and mesh depth. Channel flood messages are gated by a per-cell limit with velocity-based spacing to minimize network impact while confirming real 2-way connectivity. Aggressive probe presets (Driving/Dense) are debug-only. Includes active/passive packet distinction and filtering, session management with rename/delete, batch upload with session picker, JSON export of anonymized grid data, screen-lock prevention during recording, a floating survey indicator visible from any tab, community map overlay with coverage/repeater/time filters, and a comprehensive "How It Works" guide explaining radio asymmetry, probe types, hop counts, and map legend
...
- **Watch Repeater** — Long-press a repeater in the signal bars popover to monitor it. Shows live signal bars, a packet counter with flash animation, and hex ID. Optional audible alerts with selectable tones (Note, Chime, Ping, Pop) for antenna aiming or field testing. Picker sheet supports search by name or raw hex ID
- **Discover Neighbors** — Trigger a neighbor discovery scan from the repeater status view. Polls the binary neighbors endpoint for 60 seconds with a live countdown, updating the neighbor list in real time as nodes respond
- **Heard Repeats Map (Repeat Coverage)** — Sent messages with heard repeats can be viewed on an interactive map showing each repeat's return path through repeaters. SNR-colored last-hop lines show signal quality, with cycling through individual repeats. Falls back to current GPS when the message has no saved location, with an option to save it for future reference
- **No Repeats Retry Card** — When a sent message gets no heard repeats after 5 seconds, a retry card appears on the bubble offering to resend at the next power step (when adaptive power is enabled). Repeats arriving clear the card automatically
...
- **Mesh Gateway Scoring** — Trace data identifies the best gateway repeater per cell based on reachable nodes, hop depth, and path SNR, with an adaptive cell detail card layout
